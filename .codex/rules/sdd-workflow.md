<!-- 所屬 skill（Codex 無自動 scope，僅供參考）: spex-write-spec, spex-plan, spex-task, spex-implement, spex-selfcheck, spex-pull-request, spex-schedule, spex-fixbug, spex-challenge, spex-stamp, commit-message, create-adapter -->
...
- **Tracker Adapter: `local-file`** ← spex 系列 skill 啟動時直接從本行讀取。

可選值由 `.codex/reference/adapters/` 下實際存在的 adapter 文件決定（例如 `ado`、`local-file`）。
...
分支前綴由目前生效的 Tracker Adapter 決定（見對應 `.codex/reference/adapters/<id>.md` 文件開頭的「分支前綴」宣告；例：ADO adapter → `ADO-`）。切換 adapter 時分支前綴隨之改變，不可假設固定為 `ADO-`。
...
- [ ] 驗證指令束全綠（見 `.codex/rules/commands.md`）
...
3. **各 agent 落地（強度不等價，誠實標註）**：
   - **Claude Code**：寫 `.codex/settings.json` 的 `permissions.deny`（由 harness 而非模型強制，優先序高於 ask/allow）。**最強的中層防護**。
...
> 核心命題：**做事的人不能自評**。每個階段的產出在寫進 tracker 或交棒前，先由一個全新上下文、唯讀、立場敵對的 `challenger` 逐項詰問；未過不得交棒。執行協定見 `.codex/skills/spex-challenge/SKILL.md`。
...
- **確定性先行**：進詰問前先跑 `.codex/reference/spex/scripts/task-draft-lint.mjs`（task 接點）等機器檢查，別讓 challenger 燒在格式問題上。
...
| 元件 | 實體 | 不可仿冒性質 |
|---|---|---|
| 蓋章者 | `challenger` / `verifier` subagent（`.codex/agents/`） | 全新上下文、唯讀 |
...
| 載體 | 事件流，**一律在目標 repo 之外**（Claude Code＝`~/.codex/projects/`） | 由 harness 寫入，受 PreToolUse 硬閘保護 |
...
1. **驗章者＝程式**：一律由 `.codex/skills/spex-stamp/SKILL.md` 呼叫 `challenge-audit.py` 裁定。**exit ≠ 0 → 不得寫留言、不得交棒、不得開 PR**；結果不可被 LLM 改判。
...
|---|---|---|---|
| `agent`（預設，無沙盒） | skill 直接呼叫 MCP（`TRACKER.*`） | 本 session 事件流 `~/.codex/projects/` | PreToolUse hook `spex-stamp-guard.sh --plane agent` |
...
| 中 | Claude Code（非沙盒） | 事件流由 harness 寫在 `~/.codex/projects/`；PreToolUse 硬閘（`spex-stamp-guard.sh`）擋下未驗章的 tracker 寫入與對事件流的改寫 | hook 的 Bash 比對是字面解析，對 compound／wrapper 變體不完備 |
| 無 | Codex / Copilot | 無 subagent 註冊機制、無事件流 → **沒有可驗的章** | 只剩 `task-draft-lint` 的確定性檢查；文件不得宣稱這裡有章 |

移除 hook，皆屬**有意識的防護降級**，須在當次任務鏈留痕。

---

## 教訓回收與升級

> 目標：讓**同一類錯誤不再反覆犯**。每次 Fail 不只是當下擋下，還要萃取成跟著 repo 走的持久教訓；反覆到門檻就升級為**確定性硬防護**（不依賴模型自律）。格式單一來源見 `.codex/reference/spex/lessons-template.md`。

1. **教訓檔位置**：目標專案 `.codex/lessons/`（`L-<seq>.md` ＋ `INDEX.md`）——**必須 commit、禁止寫進 `.gitignore`**；目錄 on-demand（首次寫教訓時才 `mkdir`）。Copilot 落地 `.spex/lessons/`、Codex `.codex/lessons/`。
...
4. **Recall（取回）**：各 SDD skill 啟動的「規則載入與記憶快取」前導段，多一步 grep `.codex/lessons/INDEX.md` 取 `skills:` 命中本 skill 的 `active` 教訓，把其「防護」納入本輪注意事項。**scope 到相關 skill 才不污染無關工作。**
