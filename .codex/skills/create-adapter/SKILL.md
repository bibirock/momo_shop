...
description: >-
  ...
  .codex/reference/adapters/README.md，依抽象協定引導使用者實作全部 12 個必填操作及可選擴充操作， 將結果寫回
  .codex/reference/adapters/。前置：無。後續：更新 .codex/rules/sdd-workflow.md 或其他 spex
  skills 參考。
---
...
架構設計師角色，建立或更新 Tracker Adapter 文件，讓 spex skills 透過 `TRACKER.*` 抽象介面支援新追蹤系統。**不只寫文件**：也負責把新 adapter 接上的安全可加性設定編輯（`.mcp.json`/`.codex/settings.json`、分支命名參數化）。
...
- ✅ 新增 `type → 路線` 映射
- ❌ 切換要使用的 adapter（改 `.codex/rules/sdd-workflow.md` 的 `Tracker Adapter:` 一行）
...
讀 `.codex/reference/adapters/README.md`，理解：
...
| 情況                             | 路徑                                                     |
| --------------------------------- | ---------------------------------------------------------- |
| 只有單一 `.md`                   | `.codex/reference/adapters/<id>.md`                     |
| 含子說明 / 範本 / 附件處理等多檔 | `.codex/reference/adapters/<id>/<id>.md` + 同層附屬文件 |
...
- `.codex/reference/adapters/azure-devops/ado.md`（多檔子目錄、含 PR / 擴充操作範本）
- `.codex/reference/adapters/local-file.md`（單檔平鋪範本）
...
寫回 `.codex/reference/adapters/README.md`。
...
2. **`.codex/settings.json`**：若此 adapter 引入新的「會**合併** PR」形狀的通道 → 在 `permissions.deny` 補一筆（限**無非合併用途**的 CLI）；若合併與否取決於引數（例如同一個工具也用來改 title），deny 表達不了，須改為確認 `.codex/reference/spex/scripts/spex-merge-guard.sh` 的 MCP 工具名比對已涵蓋該工具，未涵蓋則於 Phase 6 一併提案補上。**開立** PR 的工具不需任何設定（開 PR 屬一般流程，見 `sdd-workflow.md`「PR 開立控管」）。
3. **分支命名參數化提案**：依 Phase 2.10 蒐集的分支前綴，提案（不靜默寫入，在 Phase 6 一併展示）`.codex/rules/sdd-workflow.md`「Branch Naming」與 `.codex/reference/adapters/README.md`「`TRACKER.ensureBranch`」章節的參數化編輯（若尚未參數化）
...
即將寫入：
- <新 adapter 路徑>
- .codex/reference/adapters/README.md（若需）
- .mcp.json / .codex/settings.json 的可加性編輯（若適用）
- .codex/rules/sdd-workflow.md / adapters/README.md 的分支命名參數化（若尚未做，且提案已展示）
...
## 下一步
- 於 `.codex/rules/sdd-workflow.md` 改 `- **Tracker Adapter: <id>**` 一行切換
...
- ❌ 沒先讀 `.codex/reference/adapters/README.md`
...
- [ ] 已讀 `.codex/reference/adapters/README.md`
...
✅ Adapter 已建立 → 於 `.codex/rules/sdd-workflow.md` 改 `- **Tracker Adapter: <id>**` 一行切換 → 可開始執行 spex-\* skill（互動式）。
