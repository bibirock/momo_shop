...
> 教訓檔落地於目標專案的 `.codex/lessons/`（Copilot：`.spex/lessons/`；Codex：`.codex/lessons/`）。
...
- 目錄 `.codex/lessons/`——**必須 commit、禁止寫進 `.gitignore`**（教訓要跨輪、跨人、跨工具共享，不能綁在某台機器的個人記憶層）。
- 一教訓一檔 `L-<seq>.md`（seq 為 4 位零補，如 `L-0007.md`）＋ 一份索引 `INDEX.md`。
- 目錄為 **on-demand**：首次要寫教訓時才 `mkdir -p .codex/lessons/`，不由 installer 預建。
...
| **Recall 取回** | 各 SDD skill 啟動「規則載入」前導段，多一步 grep `.codex/lessons/INDEX.md` 取 `skills:` 命中本 skill 的 `active` 教訓，讀其「防護」 | 各 SDD skill 前導段 |
...
> 詳見 `.codex/rules/sdd-workflow.md` 的 `## 教訓回收與升級` 章節（治理單一來源）。
