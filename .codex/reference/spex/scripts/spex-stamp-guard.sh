...
#   2. 任何 Write / Edit / Bash 觸及 ~/.codex/projects/（章源所在）一律擋下——
...
# ── (2) 章源保護：事件流不可被執行者改寫 ────────────────────────────────
TRANSCRIPT_MARK='/.codex/projects/'
case "$tool_name" in
  Write | Edit | MultiEdit)
    fp="$(jq -r '.tool_input.file_path // empty' <<<"$input" 2>/dev/null || true)"
    case "$fp" in
      *"$TRANSCRIPT_MARK"*)
        echo "spex 章戳硬閘：禁止改寫 ${fp}——~/.codex/projects/ 是章戳鏈的事件流（章源），由 harness 寫入；可改寫即無章可信。" >&2
        exit 2
        ;;
    esac
    ;;
    ...
    case "$cmd" in
      *"$TRANSCRIPT_MARK"*)
        ...
        case "$cmd" in
          ...
          *)
            echo "spex 章戳硬閘：禁止以 Bash 改寫 ~/.codex/projects/ 內容（章戳鏈事件流）。唯讀查詢請用 cat / grep / node / python3。" >&2
            exit 2
            ;;
        esac
        ;;
...
if [ "$PLANE" = 'sandbox' ]; then
  echo "spex 章戳硬閘（沙盒平面）：含章戳宣稱的留言必須經 host relay 寫入。章落在影子流，relay 執行檔內建的驗章硬閘才是本平面的裁定者（見 .codex/reference/spex/relay-protocol.md「驗章硬閘接線契約」）；host 直接以 MCP 發含章留言等於繞過驗章 → 拒發。" >&2
  exit 2
fi
...
scripts="$repo/.codex/reference/spex/scripts"
