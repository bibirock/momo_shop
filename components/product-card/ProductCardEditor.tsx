    ...
    <form className="editor-form" onSubmit={(event) => event.preventDefault()}>
      <fieldset className="editor-section">
        ...
        <div className="editor-columns">
          <label>售價<input type="number" min="0" step="1" value={values.price} onChange={(event) => onFieldChange("price", event.target.value)} /></label>
          <label>原價<input type="number" min="0" step="1" value={values.originalPrice} onChange={(event) => onFieldChange("originalPrice", event.target.value)} /></label>
        </div>
        <label>總銷量<input value={values.salesText} onChange={(event) => onFieldChange("salesText", event.target.value)} /></label>
        <label>評論數<input type="number" min="0" step="1" value={values.reviewCount} onChange={(event) => onFieldChange("reviewCount", event.target.value)} /></label>
      ...
      <fieldset className="editor-section">
        ...
        <label>圓角<input type="number" min="0" max="24" step="1" value={values.borderRadius} onChange={(event) => onFieldChange("borderRadius", event.target.value)} /></label>
      </fieldset>
    </form>
  );
}
